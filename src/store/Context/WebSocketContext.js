...
import { connectSocket, disconnect, tickerInfo, wsGetTicker } from "../actions";
import {
  orderBookInfo,
  wsGetOrderBook,
  wsGetOrderBookArr,
...
function WebSocketContext(props) {
  ...
  const connect = () => {
    ...
    ws.current.addEventListener("subscribed", () => {
  ...
  return (
    <SocketContext.Provider value={{ ws: ws.current, connect }}>
      {props.children}
    </SocketContext.Provider>
  );
}

const forwardRefWebSocketContext = forwardRef(WebSocketContext);

export default React.memo(forwardRefWebSocketContext);
